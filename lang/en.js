const LANG_EN = {
    title: "Topo GPX Viewer",
    lbl_layers: "Map Layer:",
    status_ready: "Ready.",
    status_searching: "Searching...",
    status_done: "Done.",
    status_no_match: "No match.",
    status_gps_missing: "No GPS.",
    status_gps_fetch: "Locating...",
    status_gps_error: "GPS Error.",
    status_loading: "Loading data...",
    status_error: "Error: ",
    status_no_data: "No data found.",
    input_search_ph: "Search location",
    info_title: "About Topo GPX Viewer",
    info_desc: "This tool lets you view and explore GPX routes on topographic maps with customizable track styling, distance labels, elevation stats, and more.<br>The app works on mobile devices, but is best experienced on larger screens.",
    info_creator: "Creator",
    lbl_version: "Version",
    info_changelog_title: "Changelog",
    info_privacy: "This application is fully client-side. It runs directly in your browser and no data or searches are saved on any server.",
    btn_close: "Close",
    modal_api_title: "Enter API Key for {service}",
    modal_api_text: "To use {service}, you need an API key. You can get one for free by registering at the link below.",
    input_api_ph: "Paste your key here...",
    btn_save: "Save",
    btn_cancel: "Cancel",
    msg_api_alert: "You must enter a key.",
    layer_lm_map: "Lantmäteriet (Sweden)",
    layer_satellite: "Satellite",
    update_available: "A new version is available.",
    update_btn: "Update",
    btn_install_app: "📲 Install as App",
    mobile_install_msg: "Install Topo GPX Viewer as an app!",
    btn_install: "Install",
    btn_tutorial: "🎓 Start Tutorial",
    tutorial_welcome_title: "Welcome to Topo GPX Viewer!",
    tutorial_welcome_text: "Let's take a quick tour of the app. Click Next to begin.",
    tutorial_language_title: "Language",
    tutorial_language_text: "Switch the app language using this button.",
    tutorial_info_title: "Info & Changelog",
    tutorial_info_text: "Tap this button to open the info panel. Here you can read about the latest changes in the changelog, and start this tutorial again at any time.",
    tutorial_minimize_title: "Minimize Panel",
    tutorial_minimize_text: "Click this button to minimize the control panel. You can also minimize it by clicking outside the control panel on the map.",
    tutorial_layers_title: "Map Layers",
    tutorial_layers_text: "Switch between different map styles: topographic, satellite, and more. Some layers require a free API key.",
    tutorial_routes_title: "Add Routes (GPX)",
    tutorial_routes_text: "Load a GPX file to overlay a route on the map. Customize track color, width, and toggle distance labels, slope coloring, waypoints, min/max elevation markers, and an interactive elevation profile. Enable map sync to center the map on your position as you scrub the profile.",
    tutorial_search_title: "Search & Location",
    tutorial_search_text: "Search for any place by name or coordinates. Use the GPS button to jump to your current location.",
    tutorial_tips_title: "💡 Tips",
    tutorial_tips_text: "• Load a GPX file to see your route with distance labels and elevation stats.\n• Use slope coloring to visualize uphills and downhills.\n• Rotate the map by holding Ctrl and dragging, or use two-finger rotation on touch devices. Tap the compass icon to reset north.\n• In the elevation profile, use ← → arrow keys to step along the track (hold Shift for bigger steps). Press Escape to clear.",
    tutorial_btn_next: "Next",
    tutorial_btn_prev: "Back",
    tutorial_btn_finish: "Finish",
    section_routes_title: "📂 Add Routes",
    btn_gpx: "📂 Load GPX Route",
    btn_gpx_clear: "✕ Clear Route",
    status_gpx_loaded: "GPX route loaded ({n} points).",
    status_gpx_error: "Failed to load GPX file.",
    status_gpx_empty: "No track data found in GPX file.",
    status_gpx_cleared: "GPX route cleared.",
    lbl_track_color: "Track Color:",
    lbl_track_width: "Track Width:",
    lbl_km_labels: "Show Distance labels",
    lbl_color_slope: "Color by slope",
    lbl_show_waypoints: "Show Waypoints",
    lbl_show_minmax: "Show Max/Min Height",
    unit_km: "Kilometers",
    unit_mi: "Miles",
    gpx_info_length: "Length",
    gpx_info_gain: "Elevation Gain",
    gpx_info_loss: "Elevation Loss",
    gpx_info_min_elev: "Min Elevation",
    gpx_info_max_elev: "Max Elevation",
    gpx_start: "Start",
    gpx_end: "End",
    elevation_profile: "Elevation Profile",
    lbl_show_elev_profile: "Show Elevation Profile",
    lbl_elev_map_sync: "Sync Map with Profile",
    lbl_show_crosshair: "Show Crosshair",

    // Edit mode
    section_edit_title: "✏️ Edit Route",
    btn_new_route: "✏️ New Route",
    btn_edit: "✏️ Edit",
    btn_done_edit: "✓ Done",
    btn_cancel_edit: "Cancel",
    btn_save_gpx: "💾 Save & Export GPX",
    btn_add_waypoint: "📍 Add Waypoint",
    btn_undo: "↩ Undo",
    btn_redo: "↪ Redo",
    lbl_gpx_name: "Route Name:",
    lbl_activity: "Activity:",
    lbl_routing: "Road routing (ORS)",
    lbl_straight_lines: "Straight lines (no routing)",
    opt_hiking: "Hiking",
    opt_cycling: "Cycling",
    opt_running: "Running",
    opt_driving: "Driving",
    ph_gpx_name: "Enter route name...",
    ph_waypoint_name: "Waypoint name",
    ph_waypoint_desc: "Description (optional)",
    btn_wp_save: "Save",
    btn_wp_delete: "Delete",
    status_routing: "Calculating route...",
    status_route_error: "Routing failed, using straight line.",
    status_route_saved: "Route saved.",
    status_route_exported: "GPX file exported.",
    status_ors_key_required: "OpenRouteService API key required for routing.",
    status_edit_mode: "Edit mode — click map to add points.",
    status_waypoint_mode: "Click map to place waypoint.",
    status_anchor_deleted: "Anchor point deleted.",
    status_rerouting: "Rerouting segments...",
    status_save_storage_full: "Storage full. Could not save route.",
    modal_ors_title: "Enter API Key for OpenRouteService",
    modal_ors_text: "To use road routing, you need a free OpenRouteService API key. Register at the link below.",
    tooltip_drag_anchor: "Drag to reroute",
    tooltip_delete_anchor: "Right-click to delete",
    tooltip_ghost_anchor: "Drag to insert point",
    confirm_cancel_edit: "Discard changes?",
    lbl_saved_routes: "Saved Routes:",
    btn_load_saved: "Load",
    btn_delete_saved: "✕",
    no_saved_routes: "No saved routes.",
};
